...
function syncAiPromptHintMeta(value) {
  syncLimitedPromptHintMeta("aiPromptHintMeta", value);
}

function syncLimitedPromptHintMeta(metaId, value) {
  const meta = $(metaId);
  if (!meta) return;
...
function bindSidebarToggleField(inputId, stateKey) {
  const input = $(inputId);
  if (!input) return;
...
function bindSidebarPromptHintField(inputId, metaId, stateKey) {
  const input = $(inputId);
  if (!input) return;
  const normalizedHint = sanitizeAiPromptHint(state[stateKey] || "");
  state[stateKey] = normalizedHint;
  input.value = normalizedHint;
  syncLimitedPromptHintMeta(metaId, normalizedHint);
  ...
  input.onpaste = (event) => {
    ...
    syncLimitedPromptHintMeta(metaId, nextValue);
    scheduleStateSave();
  };
  input.oninput = () => {
...
function renderSidebar(data) {
  ...
  bindSidebarPromptHintField("hudAiPromptHint", "hudAiPromptHintMeta", "hud_ai_prompt_hint");
  renderBgm(data);
...
function renderTextSettings() {
  ...
  const refreshPreview = debounce(refreshAllPreviews, 250);
  fieldMap.forEach(([id, stateKey]) => {
    const input = $(id);
    if (!input) return;
    ...
    input.oninput = () => {
      ...
      scheduleStateSave();
      if (["headline", "subhead", "descriptionText", "followText", "hudText"].includes(id)) {
        refreshPreview();
      }
    };
  });
...
function collectState() {
  ...
  return {
    ...
    mining_bgm_volume: Number.isFinite(miningBgmVolume) ? miningBgmVolume : (settings.mining_bgm_volume ?? 1),
    library_bgm_volume: Number.isFinite(libraryBgmVolume) ? libraryBgmVolume : (settings.library_bgm_volume ?? 0.35),
    ...
    description_ai_enabled: Boolean($("descriptionAiEnabled")?.checked),
    description_ai_prompt_hint: sanitizeAiPromptHint($("descriptionAiPromptHint")?.value || state.description_ai_prompt_hint || ""),
    follow_text: endingCopyText,
    follow_text_ai_enabled: Boolean($("followTextAiEnabled")?.checked),
    follow_text_ai_prompt_hint: sanitizeAiPromptHint($("followTextAiPromptHint")?.value || state.follow_text_ai_prompt_hint || ""),
    hud_text: $("hudText")?.value || "",
