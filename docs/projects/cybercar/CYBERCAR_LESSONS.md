...
- Symptom: removing `��ҳ` in one layer still left visible `��ҳ` buttons on some cards.
- Root cause: multiple card paths injected `��ҳ` independently (builder helpers and outgoing serialization), so single-point filtering was insufficient.
...
- Symptom: operator clicked `ˢ�¶�ά��` and observed no visible response in Telegram while browser login state was changing.
